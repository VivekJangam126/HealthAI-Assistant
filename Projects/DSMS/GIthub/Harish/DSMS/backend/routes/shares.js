...
router.post('/create', requireAuth, async (req, res) => {
  ...
  try { await sendNotification(req.user.email, 'share', `You shared ${fileName} with ${recipientEmail}`, fileName, req.user.email); } catch (e) {}
  try { await sendNotification(recipientEmail, 'share', `${req.user.email} shared a document with you: ${fileName}`, fileName, req.user.email); } catch (e) {}
...
router.post('/revoke/:shareId', requireAuth, async (req, res) => {
  ...
  // Notify both parties
  try { await sendNotification(req.user.email, 'revoke', `You revoked access to ${fileName}`, fileName, req.user.email); } catch (e) {}
  try { await sendNotification(data.recipientEmail, 'revoke', `Access to ${fileName} was revoked`, fileName, req.user.email); } catch (e) {}
...
router.get('/view/:token', async (req, res) => {
  try {
    ...
    // Notify owner about view
    (async () => {
      try {
        let ownerEmail = share.ownerUid;
        try {
          const userSnap = await firestore.collection('users').doc(share.ownerUid).get();
          if (userSnap.exists && userSnap.data().email) ownerEmail = userSnap.data().email;
        } catch (e) { /* ignore */ }
        await sendNotification(ownerEmail, 'view', `${share.recipientEmail || 'Someone'} viewed your shared file ${doc.fileName}`, doc.fileName, share.recipientEmail || null);
      } catch (e) { console.error('Notification (view) failed', e); }
    })();
...
router.get('/download/:token', async (req, res) => {
  try {
    ...
    // Notify owner about download (non-blocking)
    try {
      let ownerEmail = share.ownerUid;
      try {
        const userSnap = await firestore.collection('users').doc(share.ownerUid).get();
      ...
      await sendNotification(ownerEmail, 'download', `${share.recipientEmail || 'Someone'} downloaded your shared file ${doc.fileName}`, doc.fileName, share.recipientEmail || null);
    } catch (e) { console.error('Notification (download) failed', e); }
