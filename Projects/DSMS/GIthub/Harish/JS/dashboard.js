...
(function () {
  ...
  document.addEventListener('DOMContentLoaded', () => {
    // load profile (first name) as early as possible
    fetchProfile().catch(() => {});
  ...
  async function fetchProfile() {
    try {
      ...
      if (res.status === 401) return; // not logged in here
      if (!res.ok) return;
      const p = await res.json();
      const full = p.name || p.fullName || p.email || '';
      let first = '';
      if (full) first = String(full).trim().split(' ')[0];
      if (!first && p.email) first = String(p.email).split('@')[0];
      const el = document.getElementById('welcome-firstname');
      if (el && first) el.textContent = first;
    } catch (e) {
      // silent
    }
  }
  ...
  function setupOrUpdateCharts(chartData) {
    ...
    // Shares chart (now a filled line for smoother trend)
        ...
        charts.shares = new Chart(ctxS.getContext('2d'), {
          ...
          data: {
            ...
            datasets: [{
              ...
              data: sharesValues,
              borderColor: violet,
              backgroundColor: hexToRgba(violet, 0.12),
              tension: 0.32,
              fill: true,
              pointRadius: 2
            }]
          },
          options: smallChartOptions({ animate: true })
        });
      }
    }
  ...
  function smallChartOptions(opts = {}) {
    ...
    if (opts.animate) base.animation = { duration: 800, easing: 'easeOutCubic' };
    return base;
  }
